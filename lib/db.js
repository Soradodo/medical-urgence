...
export async function initDB() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS fiches (
      ...
      notes TEXT,
      photo_url TEXT,
      supprime_le TIMESTAMP DEFAULT NULL,
...
export async function getAllFiches() {
  ...
  const rows = await sql`
    ...
    FROM fiches WHERE supprime_le IS NULL ORDER BY created_at DESC
  `;
  return rows;
}
...
// Créer une fiche vierge (pour une puce NFC à réclamer plus tard)
export async function createBlankFiche(token) {
  const sql = getSQL();
  await initDB();
  const rows = await sql`
    INSERT INTO fiches (token, actif, reclamee)
    VALUES (${token}, false, false)
    RETURNING *
  `;
  return rows[0];
}

// Récupérer une fiche par token SANS filtrer sur actif (utilisé pour la réclamation)
export async function getFicheByTokenRaw(token) {
  const sql = getSQL();
  await initDB();
  const rows = await sql`SELECT * FROM fiches WHERE token = ${token}`;
  return rows[0] || null;
}

// Réclamer une fiche vierge (associer les infos + PIN choisis par l'utilisateur)
export async function claimFiche(token, data, hashedPin) {
  const sql = getSQL();
  await initDB();
  const rows = await sql`
    UPDATE fiches SET
      ...
      reclamee = true,
      updated_at = NOW()
    WHERE token = ${token} AND reclamee = false
    RETURNING *
  `;
  return rows[0] || null;
}
...
// Réinitialiser le PIN d'une fiche
export async function resetFichePin(id, newPin) {
  const sql = getSQL();
  await initDB();
  const hashedPin = bcrypt.hashSync(newPin, 10);
  await sql`UPDATE fiches SET pin = ${hashedPin} WHERE id = ${id}`;
}
// Supprimer une fiche
// Marquer une fiche comme supprimée (récupérable)
export async function deleteFiche(id) {
  const sql = getSQL();
  await initDB();
  await sql`UPDATE fiches SET supprime_le = NOW() WHERE id = ${id}`;
}

// Récupérer les fiches dans la corbeille
export async function getDeletedFiches() {
  const sql = getSQL();
  await initDB();
  const rows = await sql`
    SELECT id, prenom, nom, supprime_le FROM fiches
    WHERE supprime_le IS NOT NULL ORDER BY supprime_le DESC
  `;
  return rows;
}

// Restaurer une fiche depuis la corbeille
export async function restoreFiche(id) {
  const sql = getSQL();
  await initDB();
  await sql`UPDATE fiches SET supprime_le = NULL WHERE id = ${id}`;
}

// Suppression définitive (irréversible, depuis la corbeille uniquement)
export async function permanentlyDeleteFiche(id) {
  const sql = getSQL();
  await initDB();
  await sql`DELETE FROM fiches WHERE id = ${id} AND supprime_le IS NOT NULL`;
}
